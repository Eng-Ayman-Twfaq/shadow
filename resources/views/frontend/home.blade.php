...
<head>
    ...
    <title>ظل الجنوب | سواتر ومظلات</title>
...
<article class="card" data-cat="{{ $product->category }}" data-type="{{ $product->type }}" data-tags="{{ $product->tags }}">
    <div class="thumb">
        <a href="{{ route('product.show', $product->id) }}">
            <img src="{{ asset('storage/' . $product->image_url) }}" alt="{{ $product->name_ar }}">
        </a>
        <span class="badge">{{ $product->badge_text }}</span>
    </div>
    <div class="body">
        <h3><a href="{{ route('product.show', $product->id) }}" style="color: inherit;">{{ $product->name_ar }}</a></h3>
        <div class="meta"><span class="price">{{ $product->price }}</span></div>
        <div class="tags">
    ...
    <div class="actions">
        <a href="{{ route('product.show', $product->id) }}" class="btn-ghost">عرض التفاصيل</a>
...
@endforeach
    ...
    <section id="contact">
        ...
        <div class="contact">
            <form id="contact-form" action="{{ route('contact.request.store') }}" method="POST">
                @csrf
                ...
                <select name="service_type" required>
                    ...
                    <option value="مظلات حدائق">مظلات حدائق</option>
                    <option value="سواتر فلل">سواتر فلل</option>
                    <option value="سواتر مدارس">سواتر مدارس</option>
                    <option value="هناجر ساندوتش بانل">هناجر ساندوتش بانل</option>
                    <option value="بيوت شعر">بيوت شعر</option>
                    <option value="عشب صناعي">عشب صناعي</option>
                    <option value="شبوك وأسوار">شبوك وأسوار</option>
                    <option value="برجولات وتصميم حدائق">برجولات وتصميم حدائق</option>
                    <option value="استشارة أخرى">استشارة أخرى</option>
                </select>
                <textarea name="message" placeholder="تفاصيل الطلب (اختياري)"></textarea>
