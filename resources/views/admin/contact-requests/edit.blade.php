...
@section('title', 'تعديل طلب الاتصال - ' . $contactRequest->full_name)
...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">تعديل طلب الاتصال: {{ $contactRequest->full_name }}</h2>
            <p class="text-gray-600 mt-1">قم بتعديل تفاصيل طلب الاتصال المحدد</p>
        </div>
        <div class="mt-4 md:mt-0 flex space-x-3">
            <a href="{{ route('admin.contact-requests.show', $contactRequest) }}" class="inline-flex items-center px-4 py-2 bg-white text-[#1a5d7a] border border-[#1a5d7a] rounded-lg hover:bg-gray-50 transition-colors duration-200">
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden">
        <form action="{{ route('admin.contact-requests.update', $contactRequest) }}" method="POST" id="requestForm">
            @csrf
            @method('PUT')
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="mb-5">
                            ...
                            <input type="text" id="full_name" name="full_name" value="{{ old('full_name', $contactRequest->full_name) }}" 
                        ...
                        <div class="mb-5">
                            ...
                            <input type="text" id="phone" name="phone" value="{{ old('phone', $contactRequest->phone) }}" 
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1a5d7a] focus:border-[#1a5d7a] transition-colors duration-200 @error('phone') border-red-500 @enderror"
                        ...
                        <!-- نوع الخدمة -->
                        <div class="mb-5">
                            <label for="service_type" class="block text-sm font-medium text-gray-700 mb-1">نوع الخدمة</label>
                            <select id="service_type" name="service_type" 
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1a5d7a] focus:border-[#1a5d7a] transition-colors duration-200 @error('service_type') border-red-500 @enderror">
                                <option value="" disabled {{ !old('service_type', $contactRequest->service_type) ? 'selected' : '' }}>اختر نوع الخدمة</option>
                                <option value="ستائر" {{ old('service_type', $contactRequest->service_type) == 'ستائر' ? 'selected' : '' }}>ستائر</option>
                                <option value="مظلات" {{ old('service_type', $contactRequest->service_type) == 'مظلات' ? 'selected' : '' }}>مظلات</option>
                                <option value="هانجر" {{ old('service_type', $contactRequest->service_type) == 'هانجر' ? 'selected' : '' }}>هانجر</option>
                                <option value="استشارة" {{ old('service_type', $contactRequest->service_type) == 'استشارة' ? 'selected' : '' }}>استشارة</option>
                                <option value="أخرى" {{ old('service_type', $contactRequest->service_type) == 'أخرى' ? 'selected' : '' }}>أخرى</option>
                            </select>
                            @error('service_type')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <div class="mb-5">
                            ...
                            <textarea id="message" name="message" rows="5" 
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1a5d7a] focus:border-[#1a5d7a] transition-colors duration-200 @error('message') border-red-500 @enderror"
                                      placeholder="أدخل محتوى الرسالة">{{ old('message', $contactRequest->message) }}</textarea>
                            @error('message')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-5">
                            <label class="flex items-center">
                                <input type="checkbox" name="is_read" value="1" {{ old('is_read', $contactRequest->is_read) ? 'checked' : '' }} 
                                       class="h-5 w-5 text-[#1a5d7a] border-gray-300 rounded focus:ring-[#1a5d7a]">
                                <span class="mr-3 text-sm font-medium text-gray-700">الطلب مقروء</span>
                            </label>
                            @error('is_read')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <!-- تاريخ الإرسال -->
                        <div class="mb-5">
                            <label class="block text-sm font-medium text-gray-700 mb-1">تاريخ الإرسال</label>
                            <div class="bg-gray-100 p-3 rounded-lg">
                                {{ $contactRequest->submitted_at->format('d/m/Y H:i') }}
                            </div>
                        </div>
                    </div>
                </div>
...
<div id="loadingOverlay" class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 hidden">
    <div class="bg-white p-8 rounded-2xl shadow-2xl flex flex-col items-center justify-center w-80">
        ...
        <p class="text-gray-600 mt-2 text-center">يتم الآن حفظ التعديلات على الطلب، الرجاء الانتظار...</p>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        requestForm.addEventListener('submit', function(e) {
            ...
            if (!phoneField.value.trim()) {
                ...
                isValid = false;
                if (!errorMessage) errorMessage = 'رقم الهاتف مطلوب.';
            } else if (!/^\+?[0-9\s\-()]{7,20}$/.test(phoneField.value)) {
                phoneField.classList.add('input-error');
                isValid = false;
                if (!errorMessage) errorMessage = 'رقم الهاتف غير صحيح.';
            }
